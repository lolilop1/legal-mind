...
6. Тесты:
   ...
   Ожидание: 32 файла, 2298 проверок, упало 0.
...
- **Что требуете** — юзер выбирает одно требование из 9
  (вернуть деньги / заменить / отремонтировать / уценка / неустойка
  / обмен / отказаться от услуги / устранить недостатки услуги /
  вернуть предоплату). Список зависит от сценария (defect/return14/
  marketplace/service). Секция появляется после авто-детекта.
- Дата покупки
...
### Как работает выбор требования

1. Юзер пишет описание → `/detect_category` определяет сценарий
2. Возвращает список требований (`modules/consumer/demands.py`)
3. UI рендерит radio-карточки в секции «Что требуете»
4. `/submit` принимает `требование_код`
5. `engine.py` — жёстко подменяет требование от LLM выбранным
6. `pdf.py` — берёт срок и wording из формы

Справочник требований: `modules/consumer/demands.py`.
Тесты: `tests/test_consumer_demands.py` (71 проверка).
...
- **test.yml** — 2298 тестов + Playwright UI на чистой Ubuntu 24.04.
