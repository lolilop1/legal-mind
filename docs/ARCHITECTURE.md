...
6. Pre-check:
   - Подтип услуги / маркетплейса / гарантийного случая
   - Оговорка 15 дней (defect + техсложный)
   - Номер заказа для marketplace
7. Калькулятор неустойки (по сценарию + подтипу)
8. PDF: шапка (умный адресат) + тело + нормы + ТРЕБУЮ + РАСЧЁТ + подвал
9. CASE сохраняется в БД
