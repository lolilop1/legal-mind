...
4. LLM (Alice AI Flash) с конфигом сценария
5. Валидация норм по whitelist
6. Pre-check (known / critical / optional):
...
7. Калькулятор неустойки (по сценарию + подтипу):
...
8. PDF: шапка (умный адресат) + тело + нормы + ТРЕБУЮ (п. 4 — ст. 15)
       + РАСЧЁТ НЕУСТОЙКИ + подвал
9. CASE сохраняется в БД (+ calculation JSON для карточки)
10. Redirect в карточку + авто-скачивание PDF
...
### test.yml
Push → GitHub Actions → 2177 тестов на Python 3.12 + chromium (Playwright).
