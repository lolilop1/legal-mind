# Legal Mind — Changelog

## 2026-09-14 (ночь: домен + HTTPS + почта)

### Добавлено

**Домен legalmind.su**
- Зарегистрирован на reg.ru
- A-записи `@` и `www` → 201.24.49.121
- NS reg.ru, зона управляется через панель reg.ru

**HTTPS (Let's Encrypt)**
- Сертификат получен через `certbot --nginx`
- Срок действия до 13.12.2026
- Автопродление (certbot systemd timer)
- HTTP → HTTPS: 301 редирект в nginx
- nginx.conf обновлён в репо (`deploy/nginx.conf`) — для аварийного восстановления

**Secure-cookie**
- `SESSION_COOKIE_SECURE=true` в `/opt/legal_mind/web/.env`
- Cookie сессии: Secure + HttpOnly + SameSite=Lax

**Почта info@legalmind.su**
- Яндекс 360 для бизнеса
- MX: `mx.yandex.net` (приоритет 10)
- SPF: `v=spf1 redirect=_spf.yandex.net`
- DKIM: mail._domainkey (RSA 1024-bit)
- Отображаемое имя: «Legal Mind»
- Подпись: «С уважением, команда Legal Mind» + legalmind.su + info@legalmind.su

### Исправлено

- **nginx.conf после Certbot** — восстановлен rate limiting
  (`limit_req_zone general/submit`), потерянный при `certbot --nginx`
- **`.env` склейка** — `echo >>` дописал флаг к `SECRET_KEY` без
  перевода строки. Починили через Python. SECRET_KEY цел (64 hex).
- **`web/_env_backup`** — удалён из git, добавлен в `.gitignore`

### Известно

- `web/_env_backup` был в git-истории (коммит caddb56).
  Репо приватный, риск низкий.

---

## 2026-09-14 (вечер: фиксы ред-тима + выбор требования)

### Добавлено

**Фича: юзер сам выбирает требование**
- `modules/consumer/demands.py` — 9 требований (money/replace/
  exchange/repair/discount/penalty/refuse/fix_free/delivery_refund)
  со сроками и статьями
- UI: секция «Что требуете» на шаге 2 — radio-карточки
- `/detect_category` возвращает `demands` под сценарий
- `/submit` принимает `требование_код`, валидирует
- `engine.py` — жёстко подменяет требование от LLM выбранным
- `pdf.py` — берёт `требование_выбор` из формы (срок + wording)
- `tests/test_consumer_demands.py` (71 проверка)
- CSS: `.demand-cards`, `.demand-card`

**Anti-hallucination для consumer**
- `modules/consumer/entity_check.py` — `check_consumer_numeric_recall()`:
  числа и даты в формальном тексте должны присутствовать во входе
- `engine.py` — ретрай при расхождении, вырезание выживших галлюцинаций

**Криминал-фильтр**
- `hardchecks.py`: `_criminal_signal()` — STOP на «сдать краденый»,
  «подделать чек», «обмануть продавца». Жертву не трогает
- `app.py`: pre-check не запускается после `category="criminal"`

### Исправлено

- **Срок возврата денег 45→10 дн** — `_match_demand_option` в `pdf.py`
  матчит по якорным корням, а не по отдельным словам («товар» ловился
  в «принять товар в ремонт»)
- **«Продукты» для магнитофона** — `` в regex `_ITEM_PATTERNS`
  («сл**еды**» ловилось как «еды»)
- **`_CONSUMER_MARKERS`** — не ловил форму «вернуть»
- **UI: дата покупки** — убрано предзаполнение сегодняшним числом
  (`initPicker(date-purchase, false)`)
- **pre-check: аудиотехника** — магнитофон, плеер, бумбокс, диктофон
- **pre-check: fallback** — не блокирует «Товар/услуга», если подтип
  гарантийного случая уже определён
- **pre-check: маркетплейс** — без адреса продавца не блокирует
  (Avito — адрес пуст в справочнике)
- **`scenario_detect`** — приоритет `defect` над `return14` при явной
  поломке («сломался + хочу вернуть» = defect, а не return14)

### Тесты

- 2177 → 2298 проверок
- 30 → 32 файла
- Новые: `test_consumer_entity_check.py` (18), `test_consumer_demands.py` (71)

---

## 2026-09-14 (потребитель: 84 категории + авто-детект)

### Добавлено
- `modules/consumer/categories.py` — 84 категории товаров/услуг
- `detect_category(text)` — авто-детект по regex (59 тестов)
- `tech_complex` флаг (Пост. 924)
- Engine: категория → LLM-hints
- PDF: подзаголовок «о возврате стоимости смартфона» вместо «товара»
- `app.py`: `auto_detect=1` → детект категории в submit
- Endpoint `/detect_category` для UI
- UI-плашка «Определилось: Потребитель: смартфона — возврат/по браку»
- Группы: Электроника, Бытовая техника, Одежда, Обувь, Мебель,
  Косметика, Продукты, Аптека, Ювелирка, Детские, Спорт,
  Стройматериалы, Авто, Услуги

### Тесты
- 2118 → 2177
- 29 → 30 файлов

---

## 2026-09-14 (авто-детект типа + оптимизация тестов)

### Добавлено
- `modules/uk/doc_detect.py` — авто-детект типа документа по тексту
- 130+ regex-паттернов → 45 тестов (100% попаданий)
- `auto_detect=1` в форме — включает авто-выбор (без флага default `uk`)
- `tests/test_uk_configs_all.py` — все 181 конфиг за 1.5 сек
- Тайминг в `run_all.py` (общее время в конце)

### Изменено
- `test_uk_pdf_types.py` — sample 20 конфигов вместо 181 (24 → 4.6 сек)
- Полный прогон: **58 → 40 сек**
- Доки: 2118 проверок, 29 файлов

### Тесты
- 1555 → 2118
- 28 → 29 файлов

---

## 2026-09-14 (модуль УК: 181 тип документов)

### Добавлено
- **181 конфиг** в `modules/uk/configs/` (было 5)
- Категории: базовые / деньги / надзорные / соседи / инфраструктура / ремонт / безопасность / судебные / градостроительство / управление / раскрытие информации
- `modules/uk/engine.py` — общий движок (как в consumer)
- `pdf.py` — параметризован (`pdf_title`, `pdf_subtitle`, `pdf_request_block`)
- `app.py` — принимает `doc_type` из формы (default "uk")
- `_get_config()` переписан на `importlib` (без 106 if-ов)
- Тесты: `test_uk_engine.py` (114), `test_uk_pdf_types.py` (43)

### Зачем
Юзеры сталкиваются с сотней ситуаций — от холодных батарей до сноса дерева. Один УК-документ покрывал 5% случаев.

### Тесты
- 810 → 2118 проверок
- 26 → 29 файлов

---

## 2026-09-14 (мониторинг: health-check Cloud Function)

### Добавлено
- `deploy/health_check/` — Cloud Function для мониторинга прода
- Дёргает `/health` каждые 5 минут (Timer Trigger)
- При падении шлёт алерт в Telegram
- **Антифлуд** через Object Storage: state.json в S3,
  алерт только при смене статуса (ok→fail или fail→ok)
- `tests/test_health_check.py` — 21 проверка

### Зачем
Если прод упадёт — узнаём сразу, а не когда юзер напишет.
Cloud Function в Yandex Cloud, без сторонних сервисов.

---

## 2026-09-14 (anchor-norms: якорные нормы в consumer-конфигах)

### Добавлено
- `anchor_norms` во всех 4 конфигах (defect/return14/marketplace/service)
- `engine.py`: если LLM не вернула ни одной якорной нормы —
  первая добавляется принудительно (fallback, не error)
- Флаг `_anchor_fallback` в parsed для отладки
- `tests/test_consumer_engine.py` — 12 проверок

### Зачем
LLM иногда выбирает нормы из whitelist, но не те (ст. 22 вместо
ст. 18 для defect). Anchor-нормы гарантируют, что ключевая статья
сценария попадёт в PDF.

### Тесты
- 772 → 810 проверок
- 24 → 26 файлов

---

## 2026-09-14 (Lockbox для пароля шифрования бэкапов)

### Добавлено
- `core/lockbox.py` — чтение секретов из Yandex Lockbox через
  REST API (pyjwt + requests, без SDK). IAM-токен кэшируется на 11 часов
- `scripts/backup_db.py`: `_get_backup_password()` — пароль
  тянется из Lockbox, fallback на `.env.backup`
- `scripts/run_backup.sh` — bash-обёртка для cron (cd + exec,
  защита от гонки с автодеплоем)
- Yandex Cloud: секрет `legal-mind-backup-password`
  (ID: e6qoscc5gjf12n81vsf9) с ключом `BACKUP_ENCRYPTION_PASSWORD`
- Authorized key сервисного аккаунта `legal-mind-backup`
  (chmod 600, `/opt/legal_mind/sa-key.json`, в .gitignore)

### Обновлено
- GitHub Actions: `checkout@v5`, `setup-python@v6`,
  `upload-artifact@v5` (Node.js 20 → 24)

### Изменено
- `.env.backup` — теперь только `YC_S3_*`, пароль убран
- Cron `/etc/cron.d/legal-mind-backup` — зовёт `run_backup.sh`

### Грабли
- Lockbox: endpoint `payload.lockbox.api.cloud.yandex.net`,
  не `lockbox.api.cloud.yandex.net`
- `backup_db.py` не находил `core.lockbox` при запуске от sudo -
  решение через bash-обёртку `run_backup.sh`

### Тесты
- 762/762 зелёные

---

## 2026-09-14 (security: шифрование бэкапов + WAL для SQLite)

### Security

**Шифрование бэкапов cases.db:**
- `scripts/backup_db.py`: `encrypt_file()` через
  `openssl enc -aes-256-cbc -pbkdf2 -iter 100000`
- Пароль в `.env.backup` (BACKUP_ENCRYPTION_PASSWORD)
- Ключ в S3: `cases_YYYY-MM-DD_HHMMSS.db.gz.enc`
- Старые `.db.gz` (без шифрования) удалены из бакета
- Проверено: `.enc` расшифровывается, SQLite читается (12 дел)

**WAL для SQLite:**
- `core/case_db.py`: `PRAGMA journal_mode=WAL`
- `PRAGMA synchronous=NORMAL`
- Читатели не блокируют писателя — безопасно при 2 воркерах
  gunicorn + фоновый backup

### Документация
- `docs/BACKUP.md`: раздел про шифрование + обновлённая инструкция
  восстановления + «В планах: Lockbox»
- `docs/DECISIONS.md`: +1 решение
- `IDEAS.md`: банк идей (маршрутизатор, пре-документы, next_step,
  калькуляторы, узкие ниши, монетизация)

### Тесты
- 762/762 зелёные

---

## 2026-09-14 (UI: hero, карточки типа, секции формы)

### Добавлено
- **Hero-секция** на главной: крупный заголовок «Юридический документ
  за 1 минуту» с градиентом + подзаголовок + 3 галочки преимуществ
- **3 карточки выбора типа проблемы** (🏠 УК / 🔊 Тишина /
  🛒 Потребитель) вместо dropdown. Активная — синяя с галочкой.
- **Шаг 2 формы** разбит на 4 визуальные секции с иконками:
  📍 Ваш адрес / 🏢 Продавец / 🛍️ Товар или услуга /
  📅 Дата обращения
- **Цена + Текущая цена** — в 2 колонки (grid, `.form-row`)
- **`.form-section`** — скругления, focus-within подсветка,
  иконка + подзаголовок в шапке

### Исправлено
- **Air Datepicker** — больше не вылетает за карточку,
  position: 'bottom left', непрозрачный фон (был баг с `container`)

### Инструменты
- `scripts/make_screenshots.py` — 7 PNG ключевых экранов
  (главная, шаг 2, календарь открыт, чекбокс 152-ФЗ,
  карточка дела, мобилка × 2)
- `.gitignore` — `tests/screenshots/` (генерируются локально)

### Тесты
- `test_ui_playwright.py` (22 проверки) — обновлены селекторы:
  `select_option` → `label.type-card:has(input[value=...])`
- 762 проверок, 26 файлов

---

## 2026-09-14 (Security-аудит: CSRF, rate limit, 152-ФЗ, бэкапы, дедуп)

Закрыто 10 из 11 пунктов внешнего аудита.

### Добавлено

**CSRF-защита `/submit`:**
- `_get_csrf_token()` + `_check_csrf()` (hmac.compare_digest)
- Скрытое поле `_csrf_token` в форме
- Без токена → 400 + stop.html
- +5 тестов в `test_app_routes.py`

**Rate limiting (двойной слой):**
- nginx: `limit_req_zone` + `limit_req` (1 r/m на `/submit`,
  5 r/s на всё остальное)
- Python: `_rate_limit_check(ip)` — 50 дел/сутки на IP
  (in-memory dict + lock, сброс в полночь)
- +5 тестов в `test_app_security.py`

**152-ФЗ:**
- Чекбокс «Согласен(-на) на обработку ПДн» (обязательный)
- Страница `/privacy` — политика из 7 разделов
- Серверная проверка `privacy_consent`
- CSS `.privacy-consent`
- +6 тестов

**Бэкапы `cases.db`:**
- `scripts/backup_db.py`: sqlite `.backup()` → gzip → S3
- Yandex Object Storage: `legal-mind-backups`
- Cron 03:00 UTC (06:00 МСК), retention 30 дней
- Отдельный `.env.backup` (chmod 600, в .gitignore)
- `docs/BACKUP.md` — инструкция + восстановление

**Дедуп `_is_legal_entity`:**
- `modules/consumer/seller_kind.py` — единая точка правды
- Алиасы в `hardchecks.py` и `pre_checks.py`
- JS-комментарий про синхронизацию
- `tests/test_seller_kind.py` — +37 проверок

**CI/CD:**
- Actions обновлены: checkout@v5, setup-python@v6, upload-artifact@v5
  (Node.js 20 → 24)
- `test_app_routes.py` (30 проверок через test_client)
- `test_app_security.py` (+25 с rate limit)

### Тесты
- 706 → 762 проверок
- 23 → 26 файлов

### Осталось из аудита
- Шифрование `cases.db` at rest (SQLCipher) — отдельная сессия
- HTTPS — ждёт домена legalmind.su

---

## 2026-09-13 (Модуль 1: финальная полировка — ст. 15, 16, 19, 24 + 5 новых подтипов услуг)

### Добавлено

**Ст. 15 ЗоЗПП — компенсация морального вреда:**
- PDF: пункт 4 в блоке ТРЕБУЮ — «Компенсировать моральный вред
  ... в размере, определяемом судом»
- Для всех 4 consumer-сценариев

**Ст. 16 ЗоЗПП — ничтожные условия продавца:**
- pre_checks: _INVALID_CONDITIONS_RE ловит «возврат невозможен»,
  «предоплата невозвратная», «обмену и возврату не подлежит»,
  «не принимает претензии/жалобы» и т.п.
- known «Ничтожное условие продавца (ст. 16 ЗоЗПП)»
- allowed_norms +ст. 16 во всех конфигах

**Ст. 19 ЗоЗПП — срок гарантии vs 2 года:**
- pre_checks: _WARRANTY_EXPIRED_RE ловит «гарантия истекла»,
  «через год сломался», «прошёл год» и т.п.
- Для defect: known «Сроки предъявления (ст. 19 ЗоЗПП)» +
  optional «Доказательство недостатка» (нужна экспертиза)
- allowed_norms +ст. 19, llm_hints упоминают

**Ст. 24 ЗоЗПП — расчёт по текущей цене товара:**
- Поле «Текущая цена» в форме (опциональное)
- Если товар подорожал — расчёт по новой цене
- PDF: строка «(расчёт по цене на день предъявления — X вместо Y
  на день покупки, ст. 24 ЗоЗПП)»

**5 новых подтипов услуги:**
- Банковская / кредитная — ФЗ-353, ст. 16 ЗоЗПП
- Страховая — ФЗ-4015-1, ст. 28 (1%/день)
- Туристическая — ФЗ-132
- Образовательная — ФЗ-273 (ст. 32)
- Медицинская — ФЗ-323 (ст. 98)
- _detect_service_subtypes_all — возвращает ВСЕ совпадения

### Изменено
- Регекс «нарушен срок» ловит «обещали... делают третий месяц»

### Тесты
- 660 → 762 проверок (+16 за полировку М1)
- Всего 26 файлов

---

## 2026-09-13 (сессия: Модуль 1 вылизан + CI полностью автоматизирован)

Большая сессия. Модуль 1 (Потребитель) доведён до продакшн-уровня,
тесты выросли с 305 до 644, полностью переведены на CI.

### Модуль 1 — расширения

**Невозвратные товары (Пост. 2463):**
- Список категорий, не подлежащих возврату за 14 дней
- Для `return14` → STOP с объяснением; для `defect` работает ст. 18

**Техсложные товары (Пост. 924):**
- Холодильники, ноутбуки, телефоны, телевизоры, мебель и др.
- `return14` + техсложный → STOP (нельзя вернуть надлежащего качества)

**Оговорка 15 дней (ст. 18):**
- `defect` + техсложный + >15 дней → warning о существенности недостатка
- Продавец вправе предложить ремонт вместо возврата денег

**Справочник 12 маркетплейсов (реестр МЭР):**
- Ozon, Wildberries, Яндекс Маркет, Avito, Lamoda,
  Яндекс Еда/Go/Путешествия, Delivery Club, Магнит Маркет, Купер, Joom
- Авто-подстановка юрадреса владельца агрегатора
- Шапка PDF: «Владельцу агрегатора ООО «Интернет Решения» (Ozon)»

**Номер заказа для маркетплейсов:**
- Обязателен, подсказка формата для каждой площадки
- Строка «Номер заказа: …» в шапке PDF

**Услуги — 4 подтипа (ст. 27-33):**
- Нарушен срок → ст. 27, 28 (неустойка 3%/день)
- Смета превышена → ст. 33
- Отказ от услуги → ст. 32
- Некачественная работа → ст. 29, 30
- Авто-детект подтипа по тексту

**Калькулятор неустойки:**
- `modules/consumer/calculators.py`
- defect/return14/marketplace — 1%/день (ст. 22, 23)
- delivery_delay — 0.5%/день (ст. 23.1), cap = сумма предоплаты
- service — 3%/день (ст. 28), cap = цена услуги
- repair_delay — 1%/день после 45 дней (ст. 20, 23)
- Блок «РАСЧЁТ НЕУСТОЙКИ» в PDF

**Отказ в гарантийном ремонте (defect подтип):**
- Продавец не вправе отказать без экспертизы (ст. 20, 21)
- Просрочка ремонта >45 дней → неустойка 1%/день

**Просрочка доставки (marketplace подтип):**
- Товар не доставлен / задержан → ст. 23.1
- Неустойка 0.5%/день от суммы предоплаты

### UI

- Единый initPicker для двух AirDatepicker (дата покупки + обращения)
- Поле «Номер заказа» появляется только для маркетплейсов
- Inline-ошибки `.lm-field-error` красным цветом

### Тесты

- **305 → 762 проверок**, 15 → 26 файлов
- Flask-роуты (30), security-регресс (20), LLM (19), шапка PDF (25),
  контент PDF (39), UI через Playwright (19), marketplaces (61),
  calculators (64)

### CI/CD

- `.github/workflows/test.yml` — 762 тестов на каждый push + Playwright
- `.github/workflows/deploy.yml` — через `workflow_run`, только после Tests
- `.github/workflows/smoke.yml` — 6 сценариев против прода,
  понедельники 9:00 МСК + вручную
- Бейджи в README (Tests, Deploy, Smoke)
- Тесты больше не гоняются руками — всё в CI

### E2E

- `scripts/smoke_test.py --against local|prod` — 6 сценариев
- `scripts/run_smoke.ps1` — локальный хелпер с подтверждением для prod
- `test_pdf_content.py` — pypdf читает PDF, 39 проверок содержимого
- `test_ui_playwright.py` — headless Chromium тыкает календари, wizard

### Документация

- README, STATUS, ROADMAP, MODULES, ARCHITECTURE, CHANGELOG, DECISIONS,
  SETUP, web/README — все синхронизированы

---

## 2026-09-13 (smoke-тест: 6 сценариев одной командой)

### Добавлено
- scripts/smoke_test.py: поднимает Flask на 127.0.0.1:5001
  (отдельная БД через CASE_DB_PATH, прод не пачкается), отправляет
  6 реалистичных кейсов (UK, шум, consumer defect с расчётом,
  return14, marketplace Ozon, service с нарушением срока),
  сохраняет PDF, открывает их разом.
- .gitignore: smoke_output/ (артефакты теста)
- docs/SETUP.md: раздел «Smoke-тест»

### Зачем
Автоматизация ручной проверки: 5 минут кликов → 1 команда.

---

## 2026-09-13 (Модуль 1: калькулятор неустойки)

### Добавлено
- modules/consumer/calculators.py:
  - parse_amount / parse_date — парсинг цены и даты из строк
  - calculate_penalty(scenario, amount, demand_date, today) — расчёт
    неустойки по ЗоЗПП
  - build_calculation(scenario, user_data) — сборка из полей формы
  - format_rub — формат «12 345,67»
- Ставки: defect/return14/marketplace — 1%/день (ст. 22, 23);
  service — 3%/день с cap = цена услуги (ст. 28, 31)
- Срок исполнения требования — 10 дней от даты обращения

### Интеграция
- app.py: принимает «цена» и «дата_обращения»; если оба есть —
  считает неустойку и кладёт в normalized[«расчёт»]
- pdf.py: блок «РАСЧЁТ НЕУСТОЙКИ» — основное требование, неустойка
  (с формулой % × дни), итого, основание
- index.html: поля «Цена» и «Дата обращения» (Air Datepicker,
  maxDate = сегодня)
- _save_form_to_session: сохраняет цену и дату обращения

### Тесты
- tests/test_calculators.py — 52 проверки: parse_amount/date,
  calculate_penalty (defect 1%, service 3% + cap, срок не истёк),
  format_rub, build_calculation
- Всего: 17 файлов, 466 проверок (было 414)

---

## 2026-09-13 (Модуль 1: расширение норм по услугам — ст. 27-33)

### Добавлено
- configs/service.py: allowed_norms расширены до ст. 27, 28, 29,
  30, 31, 32, 33, 22 ЗоЗПП
- configs/service.py: demand_options «уплатить неустойку за нарушение
  срока» (3% за день, ст. 28) и «отказаться от услуги» (ст. 32)
- configs/service.py: llm_hints описывают 4 подтипа услуги, LLM
  выбирает нормы по фактам из описания
- pre_checks: _SERVICE_SUBTYPES — детект подтипа (нарушен срок /
  смета превышена / отказ / некачественная работа)
- pre_checks: known «Подтип услуги: …»

### Как работает
Юзер не выбирает подтип — система определяет по тексту:
- просрочка/опаздывают → ст. 27, 28 (неустойка 3%)
- смета/дороже/доплати → ст. 33
- передумал/отказываюсь → ст. 32
- халтура/дефект/отклеивается → ст. 29, 30

### Тесты
- test_consumer_pre_checks.py: +5 проверок (срок, смета, отказ,
  некачество, подтип не для defect)
- Всего: 16 файлов, 414 проверок (было 409)

---

## 2026-09-13 (Модуль 1: номер заказа + автоадрес для маркетплейсов)

### Добавлено
- marketplaces.py: get_order_hint(mp_key) — подсказка формата
  номера заказа для каждой из 12 платформ
- hardchecks: для маркетплейсов обязателен номер заказа
  (адрес подставляется из справочника, вводить вручную не нужно)
- pre_checks: known «Номер заказа» либо critical «Номер заказа»
  (для маркетплейсов)
- app.py: поле номер_заказа принимается из формы,
  адрес маркетплейса подставляется из справочника автоматически
- pdf: строка «Номер заказа: …» в шапке претензии

### Изменено
- index.html: поле «Номер заказа» появляется только для маркетплейсов,
  placeholder подсказывает формат (Ozon / WB / Lamoda и т.д.)
- index.html: JS-валидация шага 2 — маркетплейс требует номер заказа,
  а не адрес
- _save_form_to_session: сохраняются все consumer-поля (включая
  номер заказа, дату покупки, адрес/ссылку продавца) — форма не
  теряется при ошибке

### Исправлено
- pre_checks: «Продавец / исполнитель» ошибочно попадал в missing_critical
  для всех НЕ-маркетплейсов (баг после добавления marketplace-логики)

### Тесты
- test_consumer_pre_checks.py: кейс 7 разбит на 7 (с номером заказа —
  ок) и 7б (без номера — блок); +3 проверки
- test_marketplaces.py: +7 проверок (get_order_hint, hardchecks
  marketplace без/с номером, обычное ООО без адреса)
- Всего: 16 файлов, 409 проверок (было 398)

---

## 2026-09-13 (Модуль 1: справочник маркетплейсов — реестр МЭР)

### Добавлено
- modules/consumer/marketplaces.py — справочник 12 цифровых площадок
  из реестра Минэкономразвития (запуск 01.10.2026):
  Ozon, Wildberries, Яндекс Маркет, Avito, Lamoda, Яндекс Еда,
  Яндекс Go, Яндекс Путешествия, Delivery Club, Магнит Маркет,
  Купер (ex-СберМаркет), Joom
- Для каждого: brand, aliases, entity (юрлицо), ИНН, ОГРН, адрес
- resolve_marketplace(), is_marketplace(), all_aliases()

### Изменено
- hardchecks: _KNOWN_MARKETPLACES строится из справочника
  (было хардкожено)
- pre_checks: тот же источник + новый known-факт «Ответчик:
  <brand> — владелец агрегатора, <entity>»
- pdf: шапка для маркетплейсов теперь
  «Владельцу агрегатора ООО «Интернет Решения» (Ozon)»
  вместо «Продавцу Ozon»

### Тесты
- tests/test_marketplaces.py — 61 проверка (новый файл):
  реестр, resolve, is_marketplace, all_aliases, реквизиты
  Ozon/Lamoda/WB, отрицательные кейсы, уникальность алиасов
- Всего: 16 файлов, 398 проверок (было 337)

---

## 2026-09-13 (Модуль 1: оговорка 15 дней — ст. 18 ЗоЗПП)

### Добавлено
- modules/consumer/pre_checks.py: парсинг даты покупки
  (_parse_purchase_date) и логика оговорки ст. 18
- Для defect + техсложный:
  - >15 дней — добавляется known «Срок с момента покупки: N дней —
    больше 15» и опциональное предупреждение «Существенность
    недостатка» (продавец вправе предложить ремонт вместо возврата
    денег, если недостаток не существенный)
  - <=15 дней — known «в пределах 15», без предупреждения
- app.py: scenario пробрасывается в run_consumer_pre_checks
- Логика работает ТОЛЬКО для сценария defect; return14 не затрагивает

### Тесты
- test_consumer_pre_checks.py: +7 проверок (кейсы >15 дней,
  <=15 дней, return14 — 15-дневка не активна)
- Всего: 15 файлов, 337 проверок (было 330)

---

## 2026-09-13 (Модуль 1: технически сложные товары — Пост. 924)

### Добавлено
- modules/consumer/hardchecks.py: техсложные товары добавлены в
  _NON_RETURNABLE_PATTERNS (Пост. Правительства РФ от 10.11.2011 № 924):
  холодильники, стиральные/посудомоечные машины, СВЧ, электроплиты,
  кондиционеры, компьютеры, ноутбуки, телефоны, телевизоры, мебель,
  электроинструмент
- modules/consumer/pre_checks.py: известный факт «Техсложный товар»
  с пометкой оговорок ст. 18 и Пост. 924
- Логика: return14 + техсложный → STOP (нельзя вернуть надлежащего
  качества); defect + техсложный → работает по ст. 18

### Тесты
- test_consumer_hardchecks.py: +10 кейсов (return14 + техсложные,
  defect + техсложные)
- Всего: 15 файлов, 330 проверок (было 320)

---

## 2026-09-13 (Модуль 1: невозвратные товары — Пост. 2463)

### Добавлено
- modules/consumer/hardchecks.py: список невозвратных категорий
  (Постановление Правительства РФ от 31.12.2020 № 2463):
  лекарства, предметы личной гигиены, парфюмерия, ткани, бельё и
  чулочно-носочные, ювелирка, авто/мото, оружие, растения, книги,
  бытовая химия, пестициды
- hard_pre_check(user_data, scenario=None) — принимает сценарий;
  блокирует возврат невозвратного ТОЛЬКО для return14 (ст. 25),
  для defect (ст. 18) работает как раньше
- Category-маркер "non_returnable" в стопе — app.py пропускает
  pre-check для этого случая (это юридическая блокировка, а не UNKNOWN)

### Изменено
- web/app.py: process_consumer_module пробрасывает scenario в hardcheck
- web/app.py: pre-check на стопе не запускается для category=non_returnable

### Тесты
- test_consumer_hardchecks.py: +15 кейсов (невозвратные + return14,
  невозвратные + defect, legacy-вызов без scenario)
- Всего: 15 файлов, 320 проверок (было 305)

---

## 2026-09-13 (security round 2: cookie flags, ProxyFix, ПДн в логах)

### Добавлено
- web/app.py: cookie flags — SESSION_COOKIE_HTTPONLY=True, SESSION_COOKIE_SAMESITE=Lax,
  SESSION_COOKIE_SECURE (включается через .env когда появится HTTPS)
- web/app.py: ProxyFix — правильный request.remote_addr из X-Forwarded-For

### Исправлено
- web/app.py: логирование ПДн — убран адрес из log.info,
  теперь пишется только длина (addr_len) вместо значения

---

## 2026-09-13 (security: IDOR, атомарность CASE, SECRET_KEY, openai)

### Исправлено
- IDOR: /case/<ref>/pdf/<doc_id> отдавал документ по глобальному id
  без проверки принадлежности делу — теперь фильтр по (id, case_number)
- core/case_db.py: create_case — атомарная генерация case_number
  (BEGIN IMMEDIATE + retry на IntegrityError); раньше была гонка
  между воркерами gunicorn
- web/app.py: SECRET_KEY обязателен — сервис не стартует с
  небезопасным дефолтом вместо явной ошибки
- web/requirements.txt: openai>=1.50 → openai>=1.66
  (client.responses.create требует 1.66+)

### Тесты
- test_case_db.py: +1 регресс-проверка на IDOR
  (get_document_content с чужим case_number → None)
- Всего: 15 файлов, 305 проверок (по факту прогона run_all.py).
  Сразу после фикса IDOR run_all.py показывал 281 — test_case_db.py
  падал с TypeError до своей финальной строки "Total: ...", и все
  24 его проверки не попадали в агрегат. После фикса теста (передача
  case_number в get_document_content) файл долистывает до конца,
  агрegat корректный: 305. Ранее в доках заявлялось 304 — было
  до IDOR-регресс-проверки, не перепроверялось прогоном.

---

## 2026-09-12 (Модуль 1: умная шапка, валидация, нормализация)

### Добавлено
- modules/consumer/pdf.py: _format_addressee — умная шапка PDF
  - ООО/АО/ПАО → «Директору X», ИП/самозанятый → «X»
  - ФИО физлица → «Гражданину/Гражданке X» (по полу, дательный)
  - Ники → «Продавцу X»
- core/name_declension.py: decline_fio_dative, detect_gender_by_name
- core/phone_check.py: normalize_phone → «+7 (XXX) XXX-XX-XX»
- Физлица-продавцы: адрес НЕ обязателен, но нужен адрес ИЛИ ссылка
- Поле «Ссылка на профиль/объявление» в форме
- Inline-валидация: lmValidateStep, lmShowFieldError, lmMapServerErrors
- Перехват submit — проверка всех шагов, маппинг серверных ошибок в поля

### Исправлено
- Regex одинаковых цифр в телефоне
- deploy.yml: find для рекурсивного chmod

### Тесты
- test_consumer_addressee.py — 13
- test_phone_check.py — 30 (нормализация + РФ-валидация)
- Всего: 15 файлов, 304 проверки

---

## 2026-09-12 (Модуль 1: Потребитель)

### Добавлено
- modules/consumer/ — engine, hardchecks, pre_checks, pdf, scenario_detect
- 4 конфига: defect (ст. 18), return14 (ст. 25), marketplace (ст. 26.1), service (ст. 29)
- Авто-определение сценария по тексту (scenario_detect.py)
- Gender detection в PDF (проживающий/проживающей, вынужден/вынуждена)
- Wizard-форма (3 шага): проблема → детали → контакты
- Air Datepicker для даты покупки (mobile-friendly, bottom sheet)
- Авто-скачивание PDF + редирект в карточку дела (?just_created=1)
- Pre-check на STOP работает для consumer

### Изменено
- app.py: process_consumer_module + ветка consumer в submit
- core/name_declension.py: detect_gender работает и с родительным падежом
- index.html: 3-шаговый wizard вместо одной длинной формы

### Тесты
- test_consumer_hardchecks.py — 18
- test_consumer_pre_checks.py — 15
- test_consumer_scenario.py — 27
- Всего: 15 файлов, 289 проверок

---

## 2026-09-11 (доки: раздел Безопасность в README)

### Добавлено
- README.md: раздел «🔒 Безопасность» — секреты, права, деплой, CI/CD

### Удалено
- _archive/old_VPS/.env (архивный файл с ключами)

---

## 2026-09-11 (этап 5: Legal Trace)

### Добавлено
- core/trace.py — модель LegalTrace + build_trace()
- Цепочка: Факт → Квалификация → Норма → Источник
- _build_trace_dict() в app.py — собирает trace в DNA
- Блок «Как система пришла к норме» в карточке дела
- Стили .trace-chain в CSS

### Что показывает
- Модуль 2 (УК): источник — «Федеральное законодательство РФ · consultant.ru»
- Модуль 3 (шум): источник — ссылка на garant.ru (из БД регионов)

### Тесты
- test_trace.py — 16 проверок

---

## 2026-09-11 (этап 6: Версионность)

### Добавлено
- RULES_DATE = "2026.09" и TEMPLATE_VERSION = "1.0" в app.py
- Версии прокинуты в normalized для модулей 2 и 3
- PDF: подвал внизу с движком, правилами, шаблоном (серый, 8pt)
- case.html: блок «Версия документа»

---

## 2026-09-11 (этап 4: Confidence / UNKNOWN)

### Добавлено
- core/pre_checks.py — универсальная модель CheckItem/PreCheckReport
- modules/uk/pre_checks.py — проверки для жалобы в УК
- modules/noise/pre_checks.py — проверки для жалобы на шум
- Экран pre_check_blocked.html — «Что знаем / Чего не хватает»
- case.html — блок «Что знаем» в карточке дела
- Pre-check при стопе: если hard-check дал vague-стоп, но pre-check
  собрал полезный отчёт — показываем его вместо сухого stop.html

### Тесты
- test_pre_checks.py — 17 проверок

---

## 2026-09-11 (автодеплой)

### Добавлено
- GitHub репозиторий lolilop1/legal-mind (приватный)
- SSH Deploy Key на сервере
- .github/workflows/deploy.yml — GitHub Actions
- Три секрета: SSH_HOST, SSH_USER, SSH_PRIVATE_KEY
- git push → 1.5 минуты до прода

---

## 2026-09-11 (модульность)

### Изменено
- Переехали с плоской структуры на модульную
- core/, region/, modules/, web/, scripts/, tests/, docs/, deploy/
- 12 файлов тестов, 229 проверок
- systemd + nginx обновлены
- Старые плоские файлы удалены (архив в /root/)

### Тесты
- run_all.py теперь считает общее число проверок
- UTF-8 для дочерних процессов (Windows compat)

---

## 2026-09-11 (этап 3: CASE)

### Добавлено
- core/case_db.py — абстракция доступа к БД
- core/case_id.py — генератор LM-YYYYMMDD-NNNN + UUID
- core/labels.py — русские названия типов
- web/schema.sql — схема (cases, case_documents, case_events)
- Экран /my — список дел через session
- Экран /case/... — карточка дела
- Сохранение CASE после генерации PDF

### Тесты
- test_case_id.py — 17 проверок
- test_case_db.py — 23 проверки

---

## 2026-09-10 (фиксы по фидбеку тестеров)

### Исправлено
- Форма сохраняет данные при ошибке валидации
- Все ошибки показываются разом, не по одной
- Emergency-экран с кнопкой «Позвонить 112»
- Единый стиль стоп-страниц (топ-бар, кнопки)
- Нормализация адреса: «д1» → «д. 1», запятые
- Entity check: маркеры угроз (убивают, избивают, грабят)
- Промпт модуля 3: «соседи сверху» не путается с квартирой заявителя

### Тесты
- test_address.py — 7 проверок

---

## 2026-09-10 (исходная версия)

### Модуль 2 (УК)
- Hard-check (62 кейса)
- Entity recall + retry
- Три федеральные нормы
- PDF на fpdf2

### Модуль 3 (шум)
- Hard-check (19 кейсов)
- Региональные законы (85 субъектов)
- Гибридное определение региона (regex + embeddings)

### Инфраструктура
- Переход YandexGPT Pro → Alice AI Flash
- База 85 регионов
- Adversarial тесты (29/29)