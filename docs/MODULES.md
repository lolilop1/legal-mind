...
**Калькулятор неустойки (modules/consumer/calculators.py):**
- parse_amount / parse_date / calculate_penalty / build_calculation
- defect / return14 / marketplace — 1%/день от цены товара (ст. 22, 23)
- service — 3%/день, cap = цена услуги (ст. 28, 31)
- срок исполнения требования — 10 дней от даты обращения
- в PDF — блок «РАСЧЁТ НЕУСТОЙКИ» (основное + неустойка + итого)

**Услуга — 4 подтипа (ст. 27-33 ЗоЗПП):**
- единый конфиг service, авто-детект подтипа по тексту
- нарушен срок → ст. 27, 28 (неустойка 3% за день)
