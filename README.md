...
### Модуль 1 — Потребитель (4 сценария + 84 категории)

**4 сценария:** defect (ст. 18) / return14 (ст. 25) / marketplace (ст. 26.1) / service (ст. 29).

**84 категории товаров/услуг** — справочник (`modules/consumer/categories.py`):
- Электроника (11): смартфон, ноутбук, ТВ, планшет и др.
- Бытовая техника (14): холодильник, стиралка, пылесос и др.
- Одежда и обувь (12)
- Мебель (8): диван, шкаф, кровать и др.
- Косметика, продукты, лекарства
- Ювелирка, детские товары, спорт
- Стройматериалы, авто
- Услуги (11): ремонт, курсы, медицина, туризм, банк, страховка и др.

**Авто-детект категории** — `detect_category()`. Юзер пишет «купил смартфон Samsung» → система сама подставляет «смартфона» в подзаголовок PDF («о возврате стоимости **смартфона** ненадлежащего качества») и добавляет контекст в LLM-промпт.

**Выбор требования юзером** — `modules/consumer/demands.py`. Юзер сам выбирает одно требование из 9 (вернуть деньги / заменить / отремонтировать / уценка / неустойка / обмен / отказ от услуги / устранить недостатки услуги / вернуть предоплату). Срок и статья берутся из справочника, а не из LLM. Претензия не содержит «либо-либо» — только конкретное требование.

**UI-плашка** «Определилось: Потребитель: смартфона — возврат/ремонт по браку» на шаге 1.
...
**Автоматически (CI):**
- GitHub Actions гоняет все **2623 теста** на каждый push
...
**Локально (когда хочешь):**
    ...
    python tests\run_all.py               # 2623 теста, ~30 сек
...
### Защита от атак
- **Криминал-фильтр** — запросы «сдать краденое по гарантии»,
  «подделать чек», «обмануть продавца» → STOP. Жертву (покупатель
  краденого на Авито) не трогает
- **Anti-hallucination** — LLM не может выдумать числа и даты.
  `modules/consumer/entity_check.py` сверяет формальный текст
  с входом, ретраит, при нужде вырезает галлюцинации
...
### CI/CD
- Tests на каждый push (2623 проверки + Playwright)
