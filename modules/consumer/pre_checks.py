...
_ITEM_PATTERNS = [
    ...
    (r"юридическ|юрист|адвокат|нотариус",
     "Юруслуги"),
    (r"\bбанк\w*|\bкредит|микрозайм|\bмфо\b|кредитн\w*\s+карт",
     "Банковская / кредитная услуга"),
    (r"страхов\w*|осаго|каско|\bдмс\b",
     "Страховая услуга"),
    (r"\bтур\w*|пут[её]вк|туроператор|турагент|\bотел",
     "Туристическая услуга"),
    (r"\bкурс\w*|обучени|образоват|школ|репетитор",
     "Образовательная услуга"),
    (r"стоматолог|косметолог|медицинск|клиник|имплант",
     "Медицинская услуга"),
    (r"гарантийн\w*\s+ремонт|сдал\w*\s+(?:в|на)\s+ремонт|"
     r"сервис\w*\s+центр|отнёс\w*\s+в\s+сервис",
     "Гарантийный ремонт"),
]
...
_PROBLEM_PATTERNS = [
    ...
    (r"отказ\w*\s+(?:в\s+)?ремонт|не\s+гарантийн|"
     r"не\s+берут\s+в\s+ремонт|отказал\w*\s+прин(?:ять|имать)",
     "Отказ в гарантийном ремонте"),

    # Просрочка ремонта (гарантийного)
    (r"ремонт.{0,40}(?:третий\s+месяц|длится|свыше|больше).{0,20}",
     "Просрочка ремонта (>45 дней)"),

    # Навязанная услуга (банк, страховка при кредите)
    (r"навязал|навязыва|скрыт\w*\s+комисси|"
     r"дополнительн\w*\s+услуг\w*\s+при\s+кредит|"
     r"банк.{0,30}страхов",
     "Навязанная услуга (ст. 16)"),

    # Страховая не выплачивает
    (r"не\s+выплачива|отказ\w*\s+в\s+выплат|"
     r"страхов\w*\s+выплат.{0,30}(?:нет|не)",
     "Страховая не выплачивает"),

    # Смета превышена
    (r"смет.{0,30}(?:не\s+соглас|превысил|больше)|"
     r"сказал\w*.{0,30}стоить.{0,30}теперь|"
     r"требуют\s+больше\s+чем",
...
_SERVICE_SUBTYPES: tuple[tuple[str, str, str], ...] = (
    ...
    (r"банк\w*|кредит|займ|кредитн\w*\s+карт|навязал\w*\s+страхов|"
     r"скрыт\w*\s+комисси|дополнительн\w*\s+услуг\w*\s+при\s+кредит|"
     r"микрозайм|мфо\b",
     "Банковская / кредитная услуга",
     "ЗоЗПП + ФЗ-353 «О потребительском кредите» + ст. 16 — навязанные услуги"),
    # ─── Страховые ───
    (r"страхов\w*|осаго|каско|дмс|страхов\w*\s+выплат|"
     r"страхов\w*\s+компан|не\s+выплачива\w*\s+страхов",
     "Страховая услуга",
     "ЗоЗПП + ФЗ-4015-1 «Об организации страхового дела» + ст. 28 (1%/день)"),
    # ─── Туристические ───
    (r"\bтур\w*|пут[её]вк|туристическ|отел|гостиниц|"
     r"\bкурорт|авиабилет|туроператор|турагент",
     "Туристическая услуга",
     "ЗоЗПП + ФЗ-132 «Об основах туристской деятельности»"),
    # ─── Образовательные ───
    (r"\bкурс\w*|обучени|образоват|школ|репетитор|вебинар|"
     r"тренинг|мастер.?класс|интенсив|онлайн.?школ",
     "Образовательная услуга",
     "ЗоЗПП + ФЗ-273 «Об образовании» (ст. 32 — возврат)"),
    # ─── Медицинские ───
    (r"стоматолог|косметолог|медицинск|клиник|врач|"
     r"платн\w*\s+медицин|операци|процедур|"
     r"имплант|брекет",
     "Медицинская услуга",
     "ЗоЗПП + ФЗ-323 «Об основах охраны здоровья» (ст. 98)"),
    # ─── Стандартные (срок / смета / отказ / некачество) ───
    (r"просроч|позже|задерж|не в срок|срок.*наруш|"
     r"обещал\w*\s+(?:за|через|сдела|сделать|законч|заверш)|"
     r"обещал\w*[^.]{0,30}дела\w*|"
     r"должен был|опаздыва|затягив|не уложились?|"
     r"делают\s+(?:третий|второй|четвёртый|четвертый)|"
     r"третий\s+месяц\s+дела",
...
def _detect_service_subtypes_all(problem: str) -> list[tuple[str, str]]:
    """Возвращает ВСЕ подходящие подтипы услуги (не только первый)."""
    found = []
...
_INFO_RIGHT_RE = re.compile(
    r"не\s+(?:сообщил|сказал|предупредил|уведомил|проинформ)|"
    r"скрыл\w*\s+(?:информац|факт|услови)|"
    r"не\s+был\s+уведомл|"
    r"умолчал|"
    r"узнал\w*\s+(?:только\s+)?(?:потом|после|позже)",
    re.IGNORECASE,
)

# Ст. 12 ЗоЗПП — недостоверная информация о товаре/услуге
_MISINFO_RE = re.compile(
    r"недостоверн\w*\s+информац|"
    r"ввёл\w*\s+в\s+заблужд|"
    r"обман\w*|"
    r"не\s+соответству\w*\s+(?:описан|характеристик|заявлен)|"
    r"на\s+сайте\s+было\s+написан\w*",
    re.IGNORECASE,
)
...
def run_consumer_pre_checks(user_data: dict, extras: dict | None = None,
    ...
    # ─── Ст. 10: продавец не сообщил важную информацию ───
    _info_match = _INFO_RIGHT_RE.search(problem)
    if _info_match:
        add_known(
            report,
            "Нарушено право на информацию (ст. 10 ЗоЗПП)",
            f"«{_info_match.group(0)}» — продавец обязан был сообщить "
            f"эту информацию до покупки",
        )

    # ─── Ст. 12: недостоверная информация о товаре/услуге ───
    _mis_match = _MISINFO_RE.search(problem)
    if _mis_match:
        add_known(
            report,
            "Недостоверная информация (ст. 12 ЗоЗПП)",
            f"«{_mis_match.group(0)}» — потребитель вправе требовать "
            f"возмещения убытков",
        )
    ...
    _has_subtype = False
    if scenario == "service":
        subtypes = _detect_service_subtypes_all(problem)
        for label, hint in subtypes:
    ...
    if problem_kind:
        ...
        # Подтип уже определён — суть зафиксирована автоматически
        add_known(report, "Суть проблемы", "Определена по подтипу")
