...
# Детект «гарантия истекла, но 2 года не прошли» (ст. 19 ЗоЗПП)
_WARRANTY_EXPIRED_RE = re.compile(
    r"гаранти[яию].{0,30}(?:истек|закончил|прошл|кончил)|"
    r"после\s+гаранти|"
    r"гарантийн\w*\s+срок.{0,20}(?:истек|закончил|прошл)|"
    r"через\s+год\s+(?:сломал|перестал|отказ)|"
    r"через\s+два\s+года|"
    r"\bпрошл\w*\s+год|"
    r"больше\s+года\s+назад\s+куп",
    re.IGNORECASE,
)


# Ст. 16 ЗоЗПП — недействительные (ничтожные) условия продавца
_INVALID_CONDITIONS_RE = re.compile(
    r"возврат\w*\s+(?:не\s+возможен|невозмож|не\s+подлежит)|"
    r"обмен\w*\s+не\s+подлежит|"
    r"не\s+подлежит\s+(?:возврат|обмен)|"
    r"возврату\s+и\s+обмену\s+не\s+подлежит|"
    r"предоплат\w*\s+невозвратн|"
    r"предоплат\w*\s+не\s+возвращ|"
    r"магазин\s+не\s+нес[её]т\s+ответственн|"
    r"претензи[ия]\w*\s+не\s+принима|"
    r"не\s+принима\w*\s+(?:претенз|жалоб|заявлен)|"
    r"жалоб\w*\s+не\s+принима|"
    r"возврат\s+только\s+(?:в\s+течение|по)|"
    r"обмен\s+только\s+(?:в\s+течение|по)|"
    r"без\s+возврата|"
    r"возврат\s+не\s+осуществляется|"
    r"обмену\s+и\s+возврату\s+не",
    re.IGNORECASE,
)
...
def run_consumer_pre_checks(user_data: dict, extras: dict | None = None,
    ...
    _invalid_match = _INVALID_CONDITIONS_RE.search(problem)
    if _invalid_match:
        add_known(
            report,
            "Ничтожное условие продавца (ст. 16 ЗоЗПП)",
            f"«{_invalid_match.group(0)}» — недействительно, "
            f"не создаёт обязанностей для потребителя",
        )
    ...
    if scenario == "defect":
        ...
        if _WARRANTY_EXPIRED_RE.search(problem):
            add_known(
                report,
                "Сроки предъявления (ст. 19 ЗоЗПП)",
                "Гарантия истекла — требование можно предъявить "
                "в течение 2 лет, но доказывать недостаток придётся вам",
            )
            add_missing_optional(
                report,
                "Доказательство недостатка",
                "Если гарантия истекла — нужна независимая экспертиза "
                "или чек/договор для подтверждения, что недостаток "
                "производственный (ст. 19 п. 5 ЗоЗПП)",
            )
